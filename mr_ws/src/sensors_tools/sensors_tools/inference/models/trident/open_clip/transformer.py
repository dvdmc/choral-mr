...
class VisionTransformer(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        for blk in self.transformer.resblocks[-1:]:
            ...
            x = self.guidance_attn(
                blk.attn, blk.ln_1, x, token_size=token_size, **kwargs
            )
    ...
    def custom_attn(
        ...
        **kwargs
        ...
        q, k, v = F.linear(x, attn_layer.in_proj_weight, attn_layer.in_proj_bias).chunk(
            3, dim=-1
        )
        ...
        v = v.reshape(bsz * num_heads, token_size[0], token_size[1], head_dim).permute(
            0, 3, 1, 2
        )  # Batch*Head, head_dim, H, W
        v = F.interpolate(v, size=(H, W), mode="bilinear", align_corners=False)
        v = v.permute(0, 2, 3, 1).reshape(
            bsz * num_heads, H * W, head_dim
        )  # Batch*Head,H*W, head_dim

        attn_output = torch.bmm(attn_weights, v)  # Batch*Head,H*W, head_dim
        attn_output = (
            attn_output.transpose(0, 1).contiguous().view(-1, bsz, embed_dim)
        )  # LBD
        attn_output = attn_layer.out_proj(attn_output)  # LBD
        return attn_output
    ...
    def guidance_attn(self, attn_layer, norm_laryer, x, token_size=(16, 16), **kwargs):
        ...
        v = (
            v.view(-1, bsz, num_heads, head_dim).permute(1, 2, 0, 3).contiguous()
        )  # B, num_heads, L, head_dim
        v = v.permute(2, 0, 1, 3).flatten(2)  # L, B, num_heads*head_dim

        v_img = (
            v.view(vfm_token_size[0], vfm_token_size[1], bsz, embed_dim)
            .permute(2, 3, 0, 1)
            .contiguous()
        )  # B, D, H, W
        if v_img.shape[-1] % token_size[1] != 0:
            ...
            v_img = F.interpolate(
                v_img,
                size=(token_size[0] * ratio, token_size[1] * ratio),
                mode="bilinear",
                align_corners=False,
            )
        else:
        ...
        dst_v = v.new_zeros(
            1, embed_dim, kwargs.get("dst_vh") * ratio, kwargs.get("dst_vw") * ratio
        )  # 1, D, dst_h, dst_w
        count_mat = v.new_zeros(
            1, 1, kwargs.get("dst_vh") * ratio, kwargs.get("dst_vw") * ratio
        )  # 1, 1, dst_h, dst_w
        for h_idx in range(win_h):
            for w_idx in range(win_w):
                ...
                dst_v[
                    0,
                    :,
                    coord[0] * ratio : coord[2] * ratio,
                    coord[1] * ratio : coord[3] * ratio,
                ] += raw_v
                count_mat[
                    :,
                    :,
        ...
        if type(sam_v) is list:  # special case for cityscapes
            attn_weights1 = self.get_sam_sim(
                sam_v[0], sam_attn[0], cos_fac, num_heads, ref_neg
            ).type_as(v)
            attn_weights2 = self.get_sam_sim(
                sam_v[1], sam_attn[1], cos_fac, num_heads, ref_neg
            ).type_as(v)
            dst_v1, dst_v2 = (
                dst_v[:, :, :, : dst_v.shape[3] // 2],
                dst_v[:, :, :, dst_v.shape[3] // 2 :],
            )
            dst_v1 = F.interpolate(
                dst_v1,
                size=(sam_v[0].shape[2], sam_v[0].shape[3]),
                mode="bilinear",
                align_corners=False,
            )
            dst_v2 = F.interpolate(
                dst_v2,
                size=(sam_v[1].shape[2], sam_v[1].shape[3]),
                mode="bilinear",
                align_corners=False,
            )
            dst_v1 = (
                dst_v1.reshape(1, num_heads, head_dim, -1)
                .permute(0, 1, 3, 2)
                .contiguous()
            )  # 1, num_heads, dst_h*dst_w, head_dim
            dst_v2 = (
                dst_v2.reshape(1, num_heads, head_dim, -1)
                .permute(0, 1, 3, 2)
                .contiguous()
            )  # 1, num_heads, dst_h*dst_w, head_dim
            attn_output1 = torch.matmul(attn_weights1, dst_v1).view(
                1, num_heads, sam_v[0].shape[2], sam_v[0].shape[3], head_dim
            )  # 1, num_heads, dst_h, dst_w, head_dim
            attn_output2 = torch.matmul(attn_weights2, dst_v2).view(
                1, num_heads, sam_v[1].shape[2], sam_v[1].shape[3], head_dim
            )  # 1, num_heads, dst_h, dst_w, head_dim
            attn_output = torch.cat([attn_output1, attn_output2], dim=-2).flatten(2, 3)
            attn_output = (
                attn_output.permute(2, 0, 1, 3).flatten(2).contiguous()
            )  # dst_h*dst_w, 1, num_heads*head_dim
            return attn_output  # [dst_h*dst_w, 1, num_heads*head_dim]
        dst_v = F.interpolate(
            dst_v,
            size=(sam_v.shape[2], sam_v.shape[3]),
            mode="bilinear",
            align_corners=False,
        )
        attn_weights = self.get_sam_sim(
            sam_v, sam_attn, cos_fac, num_heads, ref_neg
        ).type_as(
            v
        )  # 1, num_heads, dst_h*dst_w, dst_h*dst_w

        dst_v = (
            dst_v.view(1, num_heads, head_dim, -1).permute(0, 1, 3, 2).contiguous()
        )  # 1, num_heads, dst_h*dst_w, head_dim
    ...
    def get_sam_sim(sam_v, sam_attn, cos_fac, heads, refine_neg_cos=True):
        ...
        sam_sim = torch.einsum(
            "b h m c, b h n c -> b h m n", sam_v, sam_v
        )  # 1, head, dst_h*dst_w, dst_h*dst_w
        sim_thresh = cos_fac * sam_sim.mean(dim=1, keepdim=True).mean(
            dim=-1, keepdim=True
        )
        ...
        refined_sam_attn = torch.where(
            sam_sim > sim_thresh, sam_attn, torch.zeros_like(sam_attn)
        )
        ...
        attn_weights = similarity.unsqueeze(0).repeat(
            1, heads, 1, 1
        )  # 1, num_heads, dst_h*dst_w, dst_h*dst_w
        return attn_weights
