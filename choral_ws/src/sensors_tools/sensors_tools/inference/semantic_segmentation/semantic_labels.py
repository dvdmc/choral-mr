...
def get_my_color_map(num_labels=150):
    ...
    # Ensure the colormap is of the desired size
    if num_labels > color_map.shape[0]:
        raise ValueError(
            f"Requested {num_labels} labels, but only {color_map.shape[0]} colors are available."
        )
    return color_map[:num_labels]

def get_coco_color_map(num_labels=150):
    """Returns a COCO-style color map as an np.ndarray of shape (num_labels, 3)."""
    color_map = np.array(
        [
            [0, 0, 0],
            [120, 120, 120],
