...
#include "bloomxai_map/probabilistic_map.hpp"
...
int main(int argc, char** argv) {
  cxxopts::Options options("Kitti benchmarks", "octomap VS Bloomxai");
  ...
  Bloomxai::ProbabilisticMap bloomxai_map(voxel_size);
  ...
  for (size_t count = 0; count < cloud_filenames.size(); count++) {
    ...
    bloomxai_map.insertPointCloud(pointcloud, origin, max_distance);

    const auto diff = ToMsec(std::chrono::system_clock::now() - t1);
    std::cout << "[" << filename << "] bloomxai time: " << diff << " ms" << std::endl;
    total_time_bloomxai += diff;
  }

  printf("average time bloomxai: %.1f ms\n", double(total_time_bloomxai) / double(max_pointclouds));

  std::vector<Eigen::Vector3d> bloomxai_result;
  bloomxai_map.getOccupiedVoxels(bloomxai_result);
  std::vector<Bloomxai::CoordT> free_coords;
  bloomxai_map.getFreeVoxels(free_coords);
  std::cout << "free cells: " << free_coords.size() << std::endl;
  std::cout << "bloomxai_result.pcd contains " << bloomxai_result.size() << " points\n" << std::endl;

  if (!bloomxai_result.empty()) {
    Bloomxai::WritePointsFromPCD("bloomxai_result.pcd", bloomxai_result);
  }
  ...
  if (!skip_octree) {
    ...
    if (!octree_result.empty()) {
      Bloomxai::WritePointsFromPCD("octomap_result.pcd", octree_result);
    }

    std::cout << "\nMemory used. octree: " << int(octree.memoryUsage() / 1000)
              << " Kb / bloomxai: " << int(bloomxai_map.grid().memUsage() / 1000) << " Kb" << std::endl;
    printf("speed up: %.1f X\n", double(total_time_octree) / double(total_time_bloomxai));
  }
