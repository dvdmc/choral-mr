...
class SemanticSegmentationNaradio(SemanticSegmentationBase):
  ...
  def __init__(
    ...
    model, transform = self.init_model(self.cfg.model_version, self.cfg.lang_model, self.cfg.input_resolution)
    ...
    if self.cfg.semantic_dataset_type == "custom_set":
      ...
      if self.cfg.colors is not None:
        if len(self.cfg.colors) != len(self.label_names):
          raise ValueError("colors must have the same length as custom_set_labels")
  ...
  def init_model(
    ...
    model = NARadioEncoder(
      model_version=model_version, lang_model=lang_model, input_resolution=input_resolution, compile=False
    )

    preprocess = transforms.Compose(
      [
        transforms.ToTensor(),
        transforms.Resize(model.input_resolution, interpolation=transforms.InterpolationMode.BILINEAR),
      ]
    )
  ...
  def infer(self, image):
    ...
    self.labels = (
      recover_size(probs)[0].permute(1, 2, 0).argmax(dim=-1).to(device="cpu", non_blocking=True).numpy()
    )  # Save to show if needed

    if self.semantic_feature_type == "feature_vector":
      self.semantics = recover_size(feat_map)[0].permute(1, 2, 0).to(device="cpu", non_blocking=True).numpy()
      return self.semantics

    if self.semantic_feature_type == "probability_vector":
      self.semantics = recover_size(probs).permute(1, 2, 0).to(device="cpu", non_blocking=True).numpy()
      return self.semantics
